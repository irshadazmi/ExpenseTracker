...
from app.schemas.expense_schema import ExpenseCreateSchema, ExpenseResponseSchema, ExpenseUpdateSchema
from app.services.expense_service import ExpenseService

expense_router = APIRouter()

@expense_router.get("/", response_model=list[ExpenseResponseSchema])
async def get_all_expenses(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
...
async def create_expense(expense: ExpenseCreateSchema, db: AsyncSession = Depends(get_db)):
    expense_repo = ExpenseRepository(db)
    expense_service = ExpenseService(expense_repo)
...
async def update_expense(expense_id: int, expense_data: ExpenseUpdateSchema, db: AsyncSession = Depends(get_db)):
    expense_repo = ExpenseRepository(db)
    expense_service = ExpenseService(expense_repo)
